...
int main() {
    ...
    testcases{
        ...
        ll sum=0;
        ll l = 2;
        vector<vector<ll>> cukierki(l);
        FOR(i,0,2){
            FOR(j,0,n){
                ll a;
                cin >> a;
                cukierki[i].push_back(a);
            }
        }
        ll naja = *min_element(cukierki[0].begin(), cukierki[0].end());
        ll najb = *min_element(cukierki[1].begin(), cukierki[1].end());
        FOR(i,0,n){
            sum += max(cukierki[0][i] - naja, cukierki[1][i] - najb);
        }
        cout << sum << endl;
    }
